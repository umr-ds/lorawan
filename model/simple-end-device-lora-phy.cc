  ...
  namespace lorawan
  {
                                         ...
                                         uint8_t sf, Time duration, double frequencyMHz)
    {
      ...
      case SLEEP:
      {
        ...
        if (m_device)
        {
          m_rxWrongState(packet, m_device->GetNode()->GetId());
        }
        else
        {
          m_rxWrongState(packet, 0);
        }
      ...
      case STANDBY:
      {
        ...
        if (sf != m_sf)
        {
          NS_LOG_INFO("Packet lost because it's using SF" << unsigned(sf) << ", while we are listening for SF" << unsigned(m_sf));
        ...
        {
          NS_LOG_INFO("Dropping packet reception of packet with sf = " << unsigned(sf) << " because under the sensitivity of " << sensitivity << " dBm");
